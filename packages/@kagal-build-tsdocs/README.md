...
Each JSON file contains a `DocEntry[]` array from
`tsdoc-markdown`. The `api.json` manifest includes
package name, version, generation timestamp, and
per-export metadata.
...
|--------|-------------|------|
| `@kagal/build-tsdocs` | `newDocumentsHook()`, `DEFAULT_OUTPUT_DIRECTORY`, types, `VERSION` | tsdoc-markdown, unbuild (peer) |
