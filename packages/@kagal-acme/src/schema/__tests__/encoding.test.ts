...
const samplePEMKey = [
  '-----BEGIN PUBLIC KEY-----',
  'MHcCAQEEIKjfglcVZ3aFbVtvFKd6oh+FvlKu+YQH5u/CK+UsXrqKoAoGCCqGSM49',
  'AwEHoUQDQgAEu1SU1LfVLPHCozMxH2Mo4lgOEePzNm0tRgeLezV6ffAt0gunVTLw',
  '-----END PUBLIC KEY-----',
].join('\n');
...
describe('validatePEM', () => {
  ...
  it('accepts a key block with multi-word label', () => {
    const result = validatePEM(samplePEMKey);
    expect(result.success).toBe(true);
  });
  ...
  it('rejects bare base64 without armour', () => {
    const result = validatePEM('MIIBIjANBgkqhkiG9w0BAQE=');
    expect(result.success).toBe(false);
  });
