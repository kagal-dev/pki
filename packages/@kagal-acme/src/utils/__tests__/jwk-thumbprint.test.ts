// ACME JWK thumbprint tests (RFC 7638)
...
describe('jwkThumbprint', () => {
  ...
  it('throws TypeError on unknown kty (runtime type bypass)', async () => {
    // Simulate a type-system bypass: a value with a
    // kty that isn't in the JWK union reaches
    // canonicalJWK. The default branch must throw
    // TypeError so the caller doesn't hash `undefined`
    // → a garbage thumbprint, and narrow-on-TypeError
    // catchers pick it up alongside the other failure
    // modes in canonicalJWK.
    await expect(
